...
### 5d. Optional — place screener calls via `callagent`
...
2. Generate a task brief inline based on this deal's specifics — do not use a fixed template. Author the markdown yourself, drawing on the deal context (company, the cluster's `(trigger_type, frame_id)` signature, the candidate's `match_evidence`, the wedge under evaluation) and the interview script auto-generated by `5c`. The brief should:
   ...
   - Capture the specific topic of curiosity for THIS candidate (drawn from their `match_evidence` and cluster signature)
   - List hard rules (no portfolio company name, no pitching, end on first request)
3. Write the task brief to `deals/<slug>/calls/task-screener-<candidate_id>.md` and a context file to `deals/<slug>/calls/context.md` (frontmatter with `FIRM:`, `COMPANY_DOMAIN:`, etc.). The context file can be reused across candidates within a deal.
4. Author a JSON Schema for end-of-call extraction at `deals/<slug>/calls/screener-schema.json`. Suggested fields, all optional: `specific_story_captured`, `pain_evidence`, `current_solution`, `wtp_signal`, `interesting_tangent`, `your_overall_read`. Set `"required": []`.
5. **Iterate first with simulate.** Run `callagent simulate --task deals/<slug>/calls/task-screener-<candidate_id>.md --context deals/<slug>/calls/context.md --schema deals/<slug>/calls/screener-schema.json`. Play the recipient yourself for 2-3 turns. Adjust the brief if the agent feels off.
6. Generate a consent token (e.g., output of `uuidgen`).
7. Place the real call:
   ```
   callagent place \
     --to "<candidate-phone>" \
     --task deals/<slug>/calls/task-screener-<candidate_id>.md \
     --context deals/<slug>/calls/context.md \
     --schema deals/<slug>/calls/screener-schema.json \
     --consent-token "<uuid>" \
     --output deals/<slug>/calls/<candidate_id>.json
   ```

The result file (transcript + structured data) feeds `dudu:customer-debrief`, which auto-ingests `deals/<slug>/calls/*.json` alongside transcripts under `inputs/`.
